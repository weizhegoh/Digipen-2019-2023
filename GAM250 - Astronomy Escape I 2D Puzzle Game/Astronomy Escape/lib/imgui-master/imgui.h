...
namespace ImGui
{
    ...
    IMGUI_API bool          CheckboxTick(const char* label, bool* v);
    IMGUI_API bool          CheckboxSlide(const char* label, bool* v);

    IMGUI_API bool          CheckboxFlags(const char* label, unsigned int* flags, unsigned int flags_value);
    IMGUI_API bool          RadioButton(const char* label, bool active);
    IMGUI_API bool          RadioButton(const char* label, int* v, int v_button);

//template<typename F> IMGUI_API bool Combo_lambda(const char* label, int* current_item, F items_getter, int items_count, int height_in_items = -1){
