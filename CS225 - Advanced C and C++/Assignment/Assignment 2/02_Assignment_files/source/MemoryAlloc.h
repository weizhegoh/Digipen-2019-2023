		...
		if (allocation_records)
		{
			...
			for (std::size_t i = 0; i < leak_index; ++i)
			{
				...
				if (leak.is_used)
				{
					...
					if (leak.is_array)
					{
						std::fprintf(stderr, "\n* LEAK: %zu     (array %p)", leak.size, leak.address);
					}
					else
					{
						std::fprintf(stderr, "\n* LEAK: %zu     (%p)", leak.size, leak.address);
					}
				}
			}
			if (count)
	...
	void deallocate_memory(void* address, bool is_array)
	{
		...
		if (allocation_records)
		{
				...
				if (leak.address == address)
				{
					if (leak.is_used)
					{
						if (leak.is_array)
						{
							...
							{
								issues_found = true;
								std::fprintf(stderr, "Calling delete on an array.\n");
							}
						}
						else
						{
							...
							{
								issues_found = true;
								std::fprintf(stderr, "Calling delete[] on a non-array.\n");
							}
						}
						leak.is_used = false;
						std::free(address);
			...
			if (is_array)
			{
				if (found_array)
				{
					std::fprintf(stderr, "Potential double-delete[] of an array.\n");
				}
				else if (found_non_array)
				{
					std::fprintf(stderr, "Potential double-delete[] of a non-array.\n");
				}
				else
				{
					std::fprintf(stderr, "Calling delete[] on a non-allocated memory.\n");
				}
			}
			else
			{
		...
		{
			std::fprintf(stderr, "No memory has been allocated yet.\n");
		}
	}
